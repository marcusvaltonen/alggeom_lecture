...
parameterization. First, look at the example in~\cite{larsson2017cvpr},
Section~6. Secondly, consider the following example.
...
$c_y$, $s_y$, $c_z$, and $s_z$. We nowi have an expanded system with eight unknowns and
eight equations. We will build a solver from this during the lecture.
...
\begin{enumerate}[resume]
    ...
    \item Compute a null space basis $N$ of $M$ and re-parametrizei the unknowns $X_L=NY$. If the
    rank of $M$ is $m_L$, \ie{} the equations in $F_L$ are linearly independent, $Y$ would contain
    ...
    method and the precomputed elimination template for $G(Y) = 0$ obtained by using the automatic
    generator \cite{kukelova2008}).
    \item Back-substitute to recover $X_L=NY$.
...
\mbox{$\lambda \mat{H}-\mat{R}(\mat{q})-\mat{tn}^{T}=0$} and $\mat{t}\cdot\mat{n}=0$ be denoted~$I$,
then we seek the elimination ideal~$I\cap\K[\mat{H}]$, for some suitable field~$\K$.
Over a finite field, this can be done using Macaulay2~\cite{macaulay2}.
This results in the eleven quartic constraints found
in~\cite{wadenback2016} and an additional sixth degree polynomial\footnote{See the Appendix
for implementation details.}. The constraints were symbolically verified to hold over~$\C$
as well.
In~\cite{wadenback2016} eleven quartic constraints for planar motion compatible
homographies were derived, and used to create a minimal solver.
...
With this approach, however, one cannot rule out the existence of higher order polynomials, and thus, sufficiency cannot be proven. We will approach this differently in the next Section.

In this section,
we will show that the eleven quartic constraints discussed in Section~\ref{paper03:sec:necessary} are
necessary but not sufficient. Furthermore, we show
that by adding a sixth degree polynomial to the existing quartic constraints one may guarantee
sufficiency. This is done by a novel parameterization, which allows one to efficiently compute the
relevant elimination ideal.
\end{example}
For more examples see the original publication.
...
have to use a Gröbner basis in order to generate the elimination template; it was shown
in~\cite{} that a \emph{spanning set} is enough.
