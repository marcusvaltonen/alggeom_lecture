...
on $\mat{M}(z^*)$, for a solution $z^*$, and normalize the corresponding null vector to have the
...
is because the auxiliary variable $x_0$ is present in a monomial of lower degree and therefore easier to eliminate.}
...
It turns out that the elimination template is of size~$22\times 24$.\footnote{Note that the size is not the same as in the original paper---we are using a slightly different approach. But, potentially, we have
a better solver as the state-of-the-art solver in~\cite{pritts2017} uses an elimination template of size~$24\times 26$.}
...
using Pythagorean triples, hence integer versions of the problem were obtained. By doing so,
...
\begin{example}
Study the example of relative pose with unknown focal length by~\cite{kukelova-etal-2017-cvpr}.
\end{example}
...
Gröbner basis methods only work well when the number of solutions is small---say less than one
...
with 216 and 312 solutions each. These are considered in~\cite{fabbri-etal-cvpr-2020}, and they also
