function [ eqs, data0, eqs_data ] = problem_l2_3view_triang( data0 )
...
%     t1 = randi(50,3,1);
%     t2 = randi(50,3,1);
%     E1 = R1*[0 -t1(3) t1(2);t1(3) 0 -t1(1);-t1(2) t1(1) 0];
%     E2 = R2*[0 -t2(3) t2(2);t2(3) 0 -t2(1);-t2(2) t2(1) 0];
    E1 = randi(20,3,3);
    E2 = randi(20,3,3);
    data0 = [E1(:);E2(:);randi(50,6,1)];
end

E12 = reshape(data0(1:9),3,3);
E23 = reshape(data0(10:18),3,3);
u = [reshape(data0(19:24),2,3);ones(1,3)];
...
f = sum((uh(:)-u(:)).^2)+l1*uh(:,1)'*E12*uh(:,2)+l2*uh(:,2)'*E23*uh(:,3);
eqs = diff(f);
eqs = eqs(:);

if nargout == 3
    xx = create_vars(8+18+6);
    data = xx(9:end);
    
E12 = reshape(data(1:9),3,3);
E23 = reshape(data(10:18),3,3);
u = [reshape(data(19:24),2,3);ones(1,3)];
uh = [reshape(xx(1:6),2,3);ones(1,3)];
l1 = xx(7);
l2 = xx(8);
% S = [1 0 0;0 1 0];
