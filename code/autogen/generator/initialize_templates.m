...
if opt.force_vars_in_reducibles
    all_inc(find_mon_indices(solv.reducible,setdiff(create_vars(solv.n_vars),solv.basis))) = 1;
end
if ~isempty(opt.extra_reducible)
    all_inc(find_mon_indices(solv.reducible,setdiff(opt.extra_reducible,solv.basis))) = 1;
end
...
function template = initialize_template(action, basis, reducible, all_inc, opt)
    ...
    template.desc = strrep(strrep(sprintf('action_%s',char(action)),'^',''),'*','');
